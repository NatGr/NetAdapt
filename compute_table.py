...
import argparse
import os
from utils.compute_table.wideresnet_table import compute_perf_table_wrn_2_times
...
parser.add_argument('--num_images', default=1, type=int, help='number of images the model makes predictions on at the '
                                                              'same time')
parser.add_argument('--eval_method', choices=['tf-lite-2-times'],
                    default='tf-lite-2-times', help='method used to evaluate the model')
...
parser.add_argument('--benchmark_lite_loc', default='/home/pi/tf-lite/benchmark_model', type=str,
                    help='path toward the tf-lite benchmark_model binary, see '
...
if __name__ == '__main__':
    ...
    if args.net == 'res':
        perf_table = compute_perf_table_wrn_2_times(args)
