...
class WideResNet(nn.Module):
    ...
    def __init__(self, depth, widen_factor, device, prev_model=None, num_classes=10, drop_rate=0.0):
        ...
        self.compute_metric_on = ["Conv_0"] + [f"Conv_{subnet_id}_{i}_{j}" for subnet_id in range(1, 4)
        ...
        self.all_layers = self.compute_metric_on + ["FC"] + [f"Skip_{i}" for i in range(1, 4)]
    ...
    def choose_which_channels(self, layer_name, num_channels):
        ...
        pruning_score = self._get_pruning_score(layer_name)
    ...
    def _get_pruning_score(self, layer_name):
